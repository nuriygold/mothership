...
function isValidHermesSessionId(sessionId: string | null): sessionId is string {
  if (!sessionId) return false
  return /^agent:hermes:[0-9a-fA-F-]{36}$/.test(sessionId)
}
...
export default function Hermes() {
  ...
  const cancelActiveRequest = useCallback((sessionToCancel?: string) => {
    const active = activeRequestRef.current
    if (!active) return
    if (sessionToCancel && active.sessionId !== sessionToCancel) return
  ...
  useEffect(() => {
    ...
    fetch(`/api/chat/messages?sessionId=${encodeURIComponent(sessionId)}`, { cache: 'no-store' })
      .then((r) => {
        ...
        if (!r.ok) throw new Error(`Failed to load session history (${r.status})`)
        return r.json()
      })
      .then((data) => {
        if (cancelled) return
      ...
      .catch((err: unknown) => {
        if (cancelled) return
        setError((prev) => prev ?? (err instanceof Error ? err.message : 'Failed to load session history'))
      })
  ...
  const handleSessionClose = useCallback((closedSessionId: string) => {
    cancelActiveRequest(closedSessionId)
    setMessagesBySession((prev) => {
    ...
    setLoadedSessions((prev) => {
      if (!prev.has(closedSessionId)) return prev
      const next = new Set(prev)
  ...
  async function send() {
    ...
    maybeAutoTitle('hermes', activeSessionId, text)

    const controller = new AbortController()

    try {
      cancelActiveRequest()
      activeRequestRef.current = { sessionId: activeSessionId, controller }

      const res = await fetch('/api/agent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text, sessionId: activeSessionId, agent: 'hermes' }),
      ...
      const applyStreamEvent = (dataStr: string) => {
        if (activeRequestRef.current?.controller !== controller) return true
      ...
      outer: while (true) {
        ...
        const lines = done ? chunks : chunks.slice(0, -1)
        buf = done ? '' : (chunks[chunks.length - 1] ?? '')

        for (const line of lines) {
          const trimmed = line.trim()
          if (!trimmed.startsWith('data:')) continue
          if (applyStreamEvent(trimmed.slice(5).trim())) break outer
        }
  ...
  return (
    <div style={{
      ...
      <SessionPalette agent="hermes" onSelect={handleSessionChange} />
      ...
      >
        {messages.length === 0 && !loading && (
          <div style={{ opacity: 0.5, fontSize: "14px" }}>
            Type a message below to talk to Hermes.
          </div>
        )}
