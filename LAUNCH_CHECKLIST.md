...
## 3) Deploy
- Push latest code.
- In prod env: `npm run migrate:deploy` (applies Drizzle migrations) from CI/release workflow; keep Vercel build migration disabled unless `RUN_MIGRATIONS_IN_PREVIEW=1` is intentionally set. Avoid `migrate dev` in prod.
