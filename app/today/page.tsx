    ...
    ...(item.taskId ? [
      {
        ...
        fn: async () => {
          // onComplete calls handleComplete which handles the API call + trophy
          onComplete(item.taskId!, item.title);
        },
      },
      {
        key: 'start',
      ...
      >
        ...
        >
          <div className="flex-1 min-w-0">
            <h2 className="text-sm font-semibold leading-snug" style={{ color: 'var(--foreground)' }}>{item.title}</h2>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2 scrollbar-hide">
          {actions.map((a) => (
            <button
              ...
              style={{
                background: done === a.key ? a.color : 'var(--bg-secondary, var(--muted))',
                opacity: busy && busy !== a.key ? 0.4 : 1,
              }}
            >
              <div
              ...
              >
                {busy === a.key ? (
                  <div
                    ...
                    style={{ borderColor: a.textColor, borderTopColor: 'transparent' }}
                  />
                ) : done === a.key ? (
                  <CheckCircle2 className="w-4 h-4" style={{ color: a.textColor }} />
                ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs truncate" style={{ color: 'var(--muted-foreground)' }}>{a.desc}</p>
              </div>
            </button>
          ))}
          {err && (
            <p className="text-xs px-2 pt-1" style={{ color: 'var(--destructive, #ef4444)' }}>{err}</p>
          )}
        </div>
