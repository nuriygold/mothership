...
function TrophyModal({ onClose, localCompletions }: { onClose: () => void; localCompletions: string[] }) {
  ...
  const allTasks = useMemo(() => {
    ...
    const localItems = localCompletions.map((title, i) => ({
      id: `local-${i}`, title, priority: 'high', completedAt: new Date().toISOString(),
    }));
    const seen = new Set(server.map((t) => t.title));
    return [...server, ...localItems.filter((t) => !seen.has(t.title))]
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
      ...
      <div className="w-full max-w-lg rounded-3xl flex flex-col"
        style={{ background: 'var(--card)', border: '1px solid var(--border)', maxHeight: '80vh', boxShadow: '0 24px 48px rgba(0,0,0,0.3)' }}>
        <div className="flex items-center gap-3 px-5 py-4 rounded-t-3xl flex-shrink-0"
          ...
          <button onClick={onClose} className="ml-auto w-8 h-8 rounded-xl flex items-center justify-center hover:opacity-70" style={{ background: 'rgba(0,0,0,0.06)' }}>
            <X className="w-4 h-4" style={{ color: '#0F1B35' }} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 scrollbar-hide">
          {isLoading && <div className="py-8 text-center"><div className="w-8 h-8 rounded-full border-2 border-t-transparent animate-spin mx-auto mb-2" style={{ borderColor: '#FFB800', borderTopColor: 'transparent' }} /><p className="text-sm" style={{ color: 'var(--muted-foreground)' }}>Counting your wins…</p></div>}
          {!isLoading && total === 0 && <div className="py-8 text-center"><Award className="w-12 h-12 mx-auto mb-3 opacity-20" /><p className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>Nothing completed yet today</p><p className="text-xs mt-1" style={{ color: 'var(--muted-foreground)' }}>Mark tasks done to see them here</p></div>}
          {allTasks.length > 0 && <div><p className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: 'var(--muted-foreground)' }}>Tasks Completed · {allTasks.length}</p><div className="space-y-2">{allTasks.map((task) => (
            <div key={task.id} className="flex items-center gap-3 rounded-2xl px-3 py-2.5" style={{ background: 'var(--color-mint)', border: '1px solid rgba(0,0,0,0.04)' }}>
              <CheckCircle2 className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--color-mint-text)' }} />
              <p className="text-sm font-medium truncate flex-1" style={{ color: '#0F1B35' }}>{task.title}</p>
              <span className="text-[11px] flex-shrink-0" style={{ color: 'var(--color-mint-text)', opacity: 0.75 }}>{fmtTime(task.completedAt)}</span>
            </div>))}</div></div>}
          {(data?.commands ?? []).length > 0 && <div><p className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: 'var(--muted-foreground)' }}>Gateway Commands · {data!.commands.length}</p><div className="space-y-2">{data!.commands.map((cmd) => (
            <div key={cmd.id} className="flex items-center gap-3 rounded-2xl px-3 py-2.5" style={{ background: 'var(--color-sky)', border: '1px solid rgba(0,0,0,0.04)' }}>
              ...
              {cmd.completedAt && <span className="text-[11px] flex-shrink-0" style={{ color: 'var(--color-sky-text)', opacity: 0.75 }}>{fmtTime(cmd.completedAt)}</span>}
            </div>))}</div></div>}
        </div>
        <div className="px-5 py-3 flex-shrink-0 flex items-center justify-between" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          <button onClick={onClose} className="rounded-xl px-4 py-2 text-xs font-semibold hover:opacity-80" style={{ background: '#B45309', color: '#FFFFFF' }}>Close</button>
        </div>
      </div>
    </div>
  );
