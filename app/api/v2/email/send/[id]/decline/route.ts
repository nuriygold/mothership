...
export async function POST(
  ...
  const draft = drafts.find((d) => d.tone === 'Decline');
  if (!draft) {
    return NextResponse.json({ error: 'Decline draft could not be generated.' }, { status: 500 });
  }
  ...
  publishV2Event('dashboard', 'email.reply_sent', { emailId, bot: 'Ruby', tone: 'Decline', messageId: result.messageId });
